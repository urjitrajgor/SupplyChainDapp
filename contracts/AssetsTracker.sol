...
contract AssetsTracker{
    ...
    function transferAssets(string memory _uuid, address _to ) public{
        ...
        emit AssetsTransfer(msg.sender, msg.sender, _uuid);
    }

    function getAssetDetails(string memory _uuid) public view returns(string memory, string memory, address){
        return(assetsStore[_uuid].name, assetsStore[_uuid].description, assetsStore[_uuid].manufacturer);
    }
